 ...
 * @return array
 */
function arkdin_body_classes( $classes ) {
...
add_filter( 'body_class', 'arkdin_body_classes' );

/**
 * Get tags.
 */
function arkdin_get_tag() {
    $html = '';
    if ( has_tag() ) {
        $html .= '<div class="tg-post-tag"><h5 class="tag-title">' . esc_html__( 'Post Tags :', 'arkdin' ) . '</h5>';
...
/** img alt-text **/
function arkdin_img_alt_text( $img_er_id = null ) {
    $image_id = $img_er_id;
    ...
    if ( !empty( $image_id ) ) {
        ...
        $alt_text = esc_html__( 'Image Alt Text', 'arkdin' );
    }

    return $alt_text;
}

// arkdin_offer_sidebar_func
function arkdin_offer_sidebar_func() {
    if ( is_active_sidebar( 'offer-sidebar' ) ) {

        dynamic_sidebar( 'offer-sidebar' );
    }
}
add_action( 'arkdin_offer_sidebar', 'arkdin_offer_sidebar_func', 20 );

// arkdin_service_sidebar
function arkdin_service_sidebar_func() {
    if ( is_active_sidebar( 'services-sidebar' ) ) {

        dynamic_sidebar( 'services-sidebar' );
    }
}
add_action( 'arkdin_service_sidebar', 'arkdin_service_sidebar_func', 20 );

// arkdin_portfolio_sidebar
function arkdin_portfolio_sidebar_func() {
    if ( is_active_sidebar( 'portfolio-sidebar' ) ) {

        dynamic_sidebar( 'portfolio-sidebar' );
    }
}
add_action( 'arkdin_portfolio_sidebar', 'arkdin_portfolio_sidebar_func', 20 );

// arkdin_faq_sidebar
function arkdin_faq_sidebar_func() {
    if ( is_active_sidebar( 'faq-sidebar' ) ) {

        dynamic_sidebar( 'faq-sidebar' );
    }
}
add_action( 'arkdin_faq_sidebar', 'arkdin_faq_sidebar_func', 20 );